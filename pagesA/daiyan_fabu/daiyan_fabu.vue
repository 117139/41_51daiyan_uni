	...
	<view>
		...
		<view class="container" v-if="htmlReset==0">
		  ...
		  <view class="goodsBox w100">
		    <view v-if="open_type>idx" class="quan_goods" v-for="(item,idx) in dy_fb_list" @tap="jump" :data-url="'/pages_goods/details/details?id='+item.g_id">
				...
				service.post(jkurl, data,
					function(res) {
						...
						if (res.data.code == 1) {
							...
							setTimeout(function () {
							  wx.redirectTo({
							    url: '/pagesA/daiyan_fabu_ok/daiyan_fabu_ok?activity_id='+that.dy_fb_list[0].activity_id+'&type='+that.type+'&path='+path_list,
							  })
							}, 1000)
						} else {
							// that.htmlReset=1
