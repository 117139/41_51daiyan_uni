		...
		<view v-if="datas" class="container">
			...
			<view class="h_main">
							...
							<view class="dt_li">
								<view class="dt_li_d1">
									<image v-if="item.type==2" class="dt_li_d1_img" :lazy-load='true' mode="aspectFill" :src="filter.imgIP_video(item.img[0])" :data-src="filter.imgIP_video(item.img[0])"
									  @tap.stop="jump" :data-url="'/pages_goods/my_video/my_video?uid='+uid+'&idx='+item.id"></image>
									...
									<view class="d1_msg">
										...
										<view class="dt_cz">
											<view v-if="item.img">共{{item.img.length>0?item.img.length:0}}张</view>
								...
								<view v-if="idx1<1" v-for="(item1,idx1) in item.goods" class="quan_goods por" @tap="jump" :data-url="'/pages_goods/details/details?id='+item1.g_id">
									<view class="goods_hb_box" v-if="item1.is_red">
										<!-- {{item1.is_red.is_have_share_red==1&&item1.is_red.is_may_share_red==1}}
										{{item1.is_red.is_have_share_red}}{{item1.is_red.is_may_share_red}}
										{{item1.is_red.is_have_advocacy_red==1&&item1.is_red.is_may_advocacy_red==1}}
										{{item1.is_red.is_have_advocacy_red}}{{item1.is_red.is_may_advocacy_red}} -->
				...
				<view v-if="s_type==2" class="my_list">
					<view class="sp_list">
						<view class="sp_li sp_li_v" v-for="(item,idx) in data_list" @tap="jump" :data-url="'/pages_goods/my_video/my_video?uid='+uid+'&idx='+item.a_id">
				...
				<view v-if="s_type==4" class="my_list">
					<view class="sp_list">
						<view class="sp_li sp_li_g" v-for="(item,idx) in data_list" @tap="jump" :data-url="'/pages_goods/details/details?id='+item.g_id">
							<view class="sp_li_img">
