		      ...
		      <view class="goods1" v-for="(item1,idx1) in item.goods">
		        ...
		        <view class="goodsinr">
		          ...
		          <view class="goodspri1">
					...
					<view v-if="item.is_make_plain_invoice==1||item.is_make_special_invoice==1" class="guige" 
					 :data-url="'/pagesA/order_fapiao/order_fapiao?idx='+idx+'&putong='+item.is_make_plain_invoice+'&zengzhi='+item.is_make_special_invoice+'&orderRemark='+JSON.stringify(orderRemark[idx])" 
						@tap="jump">
					  <view class="guige_l">
					    <view class="guige_l_name">开具发票</view>
					  </view>
					  <view class="dis_flex aic">
					    <view class="fz26 c9" v-if="!orderRemark[idx].invoice">
								不开票
							</view>
					    <view class="fz26 c9" v-else>
								{{orderRemark[idx].invoice.oi_type==1?'普通发票':'增值税专用发票'}}
							</view>
					    <text class="iconfont iconnext3 fz30 c9"></text>
					  </view>
					</view>
	...
	export default {
		...
		onShow() {
		  ...
		  if (currPage.data.yhlist_idx) {
			...
			//优惠
		  if (currPage.data.invoice_idx) {
				var invoice_idx=currPage.data.invoice_idx
				var invoice=currPage.data.invoice
		    
		    if(invoice==-1){
					that.$set(that.orderRemark[invoice_idx],'invoice','')
				}else{
					that.$set(that.orderRemark[invoice_idx],'invoice',invoice)
				}
				// that.order_ls(order_ls_data)
		  }
			// #endif
		...
		methods: {
			  ...
			  var that = this
				...
				if (that.my_dou_xuan) {
				 ...
				 if (zsum >my_d/100) {
					...
					my_dou_new = my_d - zsum*100
					that.use_dou=zsum*100
					zsum = 0
				 }
			...
			subbtn() {
			  ...
			  let that = this
				 var orderRemark=JSON.parse(JSON.stringify(that.orderRemark))
