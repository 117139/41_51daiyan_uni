	...
	<view>
			<swiper :vertical="true" :circular="true" :current="current" :skip-hidden-item-layout="true" @change="changeCurrent" @animationfinish="changeItem">
			    <swiper-item :item-id="oneItemParam.id">
			        <video title="1111111" id="myVideo0"  @fullscreenchange="fullScreen_fuc" :src="oneItemParam.src"  :poster="oneItemParam.pic" :custom-cache="true" :controls="false" :show-center-play-btn="true" :show-fullscreen-btn="false" @play="eventPlay" @tap="tabVideo"></video>
			    </swiper-item>
			
			    <swiper-item :item-id="twoItemParam.id">
			        <video title="2222222" id="myVideo1"   @fullscreenchange="fullScreen_fuc" :src="twoItemParam.src"  :poster="twoItemParam.pic" :custom-cache="true" :controls="false" :show-center-play-btn="true" :show-fullscreen-btn="false" @play="eventPlay" @tap="tabVideo"></video>
			    </swiper-item>
			
			    <swiper-item :item-id="threeItemParam.id">
			        <video title="3333333" id="myVideo2"   @fullscreenchange="fullScreen_fuc" :src="threeItemParam.src" :poster="threeItemParam.pic" :custom-cache="true" :controls="false" :show-center-play-btn="true" :show-fullscreen-btn="false" @play="eventPlay" @tap="tabVideo"></video>
			    </swiper-item>
			
			</swiper>
			<cover-view class="back_btn" :style="style" @tap="back()">
				<cover-image src="/static/images/back.png" class="head"></cover-image>
			</cover-view>
			<!-- 用户 -->
...
<script>
	...
	export default {
		...
		onLoad: function (e) {
		  ...
		  /* 初始化页面视频id 及 视频下标 */
		  this.videoId= e.videoId || 1,
		  this.videoList= list1
		  
		
		  let videoIndex = this.videoList.findIndex(v => v.id == this.videoId)
		  ...
		  if (current === 0) {
		    this.oneItemParam= list1[videoIndex]
		  }
		
		  if (current === 1) {
		    this.twoItemParam= list1[videoIndex]
		  }
		
		  if (current === 2) {
		    this.threeItemParam= list1[videoIndex]
		  }
