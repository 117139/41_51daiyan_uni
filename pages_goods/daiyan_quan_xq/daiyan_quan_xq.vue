<template>
	<view>
		...
		<view class="container">
			...
			<view v-if="tab_type==1&&data_list.length>0" class="list_box">
				...
				<view class="star_li" v-for="(item,idx) in data_list">
					<view class="star_msg">
						...
						<view class="user_msg">
							...
							<view class="oh1 fz22">
								<image :src="filter.imgIP('/static_s/51daiyan/images/star_jj.png')"></image>简介：{{item.content?item.content:''}}
							</view>
							<view class="">
								<image :src="filter.imgIP('/static_s/51daiyan/images/dy_num.png')"></image>代言数量：{{item.sum}}
							</view>
						</view>
		...
		onShareTimeline(){
			...
			if(that.loginMsg){
				return {
					...
					query:'pid=' + that.loginMsg.id+'&id='+that.id,
				}
			}else{
				return {
					title:'我是代言人，邀您领红包！',
