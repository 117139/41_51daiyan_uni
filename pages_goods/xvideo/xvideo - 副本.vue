<template>
	<view>
			<swiper :vertical="true" :circular="circular_type" :current="current" :skip-hidden-item-layout="false" @change="changeCurrent" @animationfinish="changeItem">
		...
		methods: {
			...
			changeCurrent(e) {
			  ...
			  if (videoIndex == (videoList.length - 2)) {
				...
				// 	// if (videoIndex == 0||videoIndex == (videoList.length - 1)) {
				// 	if (videoIndex == 0) {
				// 		that.circular_type=false
				// 	}else{
				// 		that.circular_type=true
