<template>
	<view>
		...
		<view v-if="htmlReset==2">
			...
			<view class="you_like">
				...
				<view class="goods_list">
					<view class="like_goods_li" @tap="jump" :data-url="'/pages_goods/details/details?id='+item.g_id" v-for="(item,index) in like_goods">
		...
		<view v-if="goodsData" :class="sheetshow1||sheetshow?'container-ban':'container'">
		  ...
		  <view class="goods_msg">
		    <view class="goods_pri">
		      <view class="pri1  dis_flex aic">
						<text v-if="goodsData.fk_is_way==2" class="xcxdy_zy_icon">自营</text>¥{{goodsData.current_price}}
					</view>
		    ...
		    <navigator v-if="goodsData.room_id>0" :url="'plugin-private://wx2b03c6e691cd7370/pages/live-player-plugin?room_id='+goodsData.room_id" class="goods_name">
					<text class="zb_box">直播中</text>
					<text>{{goodsData.title}}</text>
					
				</navigator>
		    <view v-else class="goods_name">
					<text>{{goodsData.title}}</text>
		  ...
		  <view class="htk mt20">
		    ...
		    <view class="goods_list">
		      <view v-if="idx<3" class="goods_li" v-for="(item,idx) in LaterBuy_list" @tap="jump" :data-url="'/pages_goods/details/details?id'+gid">
	...
	export default {
		...
		onShareAppMessage: function () {
			...
			if(that.loginMsg){
				return {
					title: '51代言',
					path: '/pages_goods/details/details?share_id=' + that.loginMsg.id+'&id='+that.g_id,
				...
				return {
					title: '51代言',
					path: '/pages_goods/details/details?id='+that.g_id,
					success: function(res) {
						console.log('成功', res)
					}
				}
		...
		onShareTimeline(){
			...
			if(that.loginMsg){
				return {
					...
					query:'pid=' + that.loginMsg.id+'&share_id=' + that.loginMsg.id+'&id='+that.g_id,
				}
			}else{
				return {
					title:'我是代言人，邀您领红包！',
			...
			nowbuy() {
			  ...
			  if (that.btnkg == 1) {
				...
				// advocacyId:that.dy_id,
					// advocacyviceId:that.advocacyviceId,
			  uni.navigateTo({
			    url: '/pagesA/Order/Order?type=1&v_id='+that.v_id+'&number='+that.cnum+'&advocacy_user_id='+that.dyr_type+'&dy_id='+that.dy_id+'&advocacyviceId='+that.advocacyviceId
			  })
			},
			bm_fuc() {
